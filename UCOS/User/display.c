		...
		{									
			if(RcvBufU1[i]==0x5a&&RcvBufU1[i+1]==0xa5&&RcvBufU1[i+3]==0x83)               //收到数据帧解析
			{
               ...
               if	((BMSMessage.RequestVoltage>750)||(BMSMessage.RequestVoltage<350))	
					...
					case DW_ADDR_REQUEST_CURRENT_INPUT_A:
					     ...
					     if(BMSMessage.RequestCurrent>=400)
							  	BMSMessage.RequestCurrent=400;
						break;	
					...
					case DW_ADDR_MAXSingleVoltage_INPUT:                                          //单体电压						
					     ...
					     if	((BMSMessage.MAXSingleVoltage>500)||(BMSMessage.MAXSingleVoltage<100))	
								  BMSMessage.MAXSingleVoltage=360;					
						break;					
					...
					case DW_ADDR_SINGLE_TEMPERATURE_INPUT:                                        //单体电压温度						 
					     ...
					     if	((BMSMessage.SingleVoltageTemp>99)||(BMSMessage.SingleVoltageTemp<1))	
								  BMSMessage.SingleVoltageTemp=50;						
						break;
					...
					case DW_ADDR_MAXSingleVoltageTemp_INPUT:                                      //单体电压温度						 
					     ...
					     if	((BMSMessage.MAXSingleVoltageTemp>99)||(BMSMessage.MAXSingleVoltageTemp<1))	
								  BMSMessage.MAXSingleVoltageTemp=30;						
						break;			
            ...
            break;
					...
					case DW_ADDR_SOCIncreaseTime_INPUT:                                           
						  ...
						  if ((Sys_PARA.SOCIncreaseTime>99)||(Sys_PARA.SOCIncreaseTime<1))
					     Sys_PARA.SOCIncreaseTime=1;				
					  break;
...
static void DisplayVariableValue(void)
{
   ...
   Dwin_ChangeVariableValue_Frame1(DW_ADDR_CHARGE_CURRENT,ChargerMsg.ChargeCurrent/100);
	...
	{
	  Dwin_ChangeVariableValue_Frame1(DW_ADDR_CHARGE_VOLTAGE,0);
    Dwin_ChangeVariableValue_Frame1(DW_ADDR_CHARGE_CURRENT,0);
	}
...
**	函数：写变量值
*********************************************************************************************************/ 
//void dspdynicon(unsigned short addr,unsigned short value, unsigned char page){
//		unsigned int datlen;
//		unsigned char dspbuf[32];
//			
//			switch(page){
//				case 0:
//					UART0SendStr(tab_stdby,sizeof(tab_stdby));
//				break;
//				case 1:
//					UART0SendStr(tab_prompt,sizeof(tab_prompt));
//				break;
//				case 2:
//					UART0SendStr(tab_work,sizeof(tab_work));
//				break;
//				case 3:
//					UART0SendStr(tab_balance,sizeof(tab_balance));
//				break;
//				
//				default:
//					break;
//			}
//			
//			memcpy(dspbuf,tab_dticon,sizeof(tab_dticon));
//			dspbuf[4] = addr/256;
//			dspbuf[5] = addr%256;
//			dspbuf[6] = 0;
//			dspbuf[7] = 0;
//			datlen = 8;
//			UART0SendStr(dspbuf,datlen);
//		}
//}
